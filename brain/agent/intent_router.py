...
class IntentRouter:
    ...
    def route(
        ...
        """
        from brain.memory.session.session_memory import get_session_memory
        ...
        if intent_value == "agent_task" or (intent_value == "multi_step" and getattr(understanding, "use_orchestrator", False)):
            ...
            task = AgentTask(
                user_request=understanding.original_text or understanding.text,
                goal=understanding.text,
                context={"parameters": understanding.parameters},
            )
            res = self.execute_agent_task(task)
            return {
                "type": "agent",
                "success": res.success,
                "message": res.summary,
    ...
    def _execute_command(self, cmd, validation) -> Dict[str, Any]:
        ...
        if validation.safety_level == SafetyLevel.NEEDS_CONFIRMATION:
            session = get_session_memory()
            from brain.security.action_validator import ValidationResult
        ...
        try:
            ...
            from brain.memory.user.long_term_memory import record_command
    ...
    def _handle_new_functionality(self, understanding, session) -> Dict[str, Any]:
        ...
        try:
            from brain.extensions.self_improver import get_self_improver
    ...
    def execute_agent_task(self, task, plan=None):
        """Route a multi-step AgentTask through the AgentOrchestrator."""
        from brain.agent.orchestrator import AgentOrchestrator
        orchestrator = AgentOrchestrator()
        return orchestrator.run(task, plan=plan)
