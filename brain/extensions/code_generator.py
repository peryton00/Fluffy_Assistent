...
try:
    ...
    _ai_src = str(Path(__file__).resolve().parent.parent / "ai" / "src")
...
class CodeGenerator:
        ...
        if self.llm_client is None:
            try:
                if get_client is not None:
                    ...
                    from brain.ai.llm_client import get_client as gc
                    self.llm_client = gc()
    ...
    def generate_intent_handler(
        ...
        # Import validator
        try:
            from brain.extensions.code_validator import validate_extension_code
        except ImportError:
            try:
...
'''
    ...
    def decide_language(self, description: str) -> str:
...
Reply with ONLY one word: python, html, or js."""
    ...
    def fix_handler(self, intent_name: str, handler_code: str, error_message: str) -> str:
...
"""
    ...
    def generate_web_ui(self, intent_name: str, description: str) -> dict:
...
<body><h1>{intent_name}</h1><p>{description}</p></body>
    ...
    def generate_js_handler(self, intent_name: str, description: str, parameters: dict) -> str:
