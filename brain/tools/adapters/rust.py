...
class RustToolAdapter(ToolAdapter):
    ...
    def discover_tools(self) -> List[ToolDefinition]:
        ...
        standard_caps = [
            ...
            ("Network.GetInterfaces", "Get detailed network interfaces with physical classification and traffic rates", ToolRiskLevel.READ_ONLY, False),
            ("Network.GetLocalDevices", "Discover local subnet neighbor devices from OS ARP cache", ToolRiskLevel.READ_ONLY, False),
            ("Network.GetActiveFlows", "Map active TCP and UDP socket flows to owning local processes", ToolRiskLevel.READ_ONLY, False),
            ("Network.ListWifiProfiles", "Enumerate known Wi-Fi network profiles and non-secret security posture", ToolRiskLevel.READ_ONLY, False),
