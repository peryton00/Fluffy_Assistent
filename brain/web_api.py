...
# ============================================================================
# NETWORK / LAN DISTRIBUTED MONITORING ENDPOINTS
# ============================================================================

@app.route("/network/role", methods=["GET"])
...
@app.route("/network/role", methods=["POST"])
...
@app.route("/network/availability/start", methods=["POST"])
...
@app.route("/network/availability/stop", methods=["POST"])
...
@app.route("/network/availability/status", methods=["GET"])
...
def admin_add_machine():
    """Add a client machine to the admin's watch list (ping to verify reachability)."""
    if request.remote_addr not in ("127.0.0.1", "::1"):
        return jsonify({"error": "Forbidden"}), 403
    token = request.headers.get("X-Fluffy-Token")
    if token != FLUFFY_TOKEN:
...
@app.route("/network/admin/remove", methods=["POST"])
...
@app.route("/network/admin/remove_all", methods=["POST"])
def admin_remove_all_machines():
    """Remove all machines and stop polling (called when leaving admin mode)."""
    if request.remote_addr not in ("127.0.0.1", "::1"):
        return jsonify({"error": "Forbidden"}), 403
    token = request.headers.get("X-Fluffy-Token")
    if token != FLUFFY_TOKEN:
...
@app.route("/network/admin/machines", methods=["GET"])
...
@app.route("/network/admin/switch", methods=["POST"])
...
@app.route("/network/admin/data/<machine_id>", methods=["GET"])
